...
TEST(common, getFileListFromFile){
      ...
      std::vector<std::string> mustFileList;
      mustFileList = {"test/filesForTest/Kany West - Power.mp3","test/filesForTest/Arctic Monkey - Old Yellow Bricks.mp3"};
      EXPECT_EQ (theresFileList, mustFileList);
}
...
TEST(common, keyInfoParametrs){
  ...
  //char keyf[] = "-f", file[] = "test/filesForTest/testPath.txt";
  char keyi[] = "-i";
  char *argv[] = {nullptr, /*keyf, file,*/ keyi};
  EXPECT_EQ(input(sizeof(argv)/sizeof(argv[0]), argv, outFileList),0);
  EXPECT_TRUE(info);
}
